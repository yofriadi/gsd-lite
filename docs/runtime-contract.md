...
Captured from the interview/explore phase and persisted at the start of every `validate-plan` call. The `planningContext` parameter uses the `PlanningContext` shape; the persisted `gsd-planning-context` entry adds the matching review `iteration`. The planning context is the contract against which the `plan-reviewer` judges objective alignment, constraint compliance, and scope control.
...
interface GsdReviewResult {
   ...
   * `gsd-stored-candidate-plan` entry and refuses the cycle as a `parse`
...
- the plan the reviewer scores and the plan `validate-plan` persists are the same bytes because both reference the `gsd-stored-candidate-plan` entry; `validate-plan` does not take an inline plan
...
During `/gsd-plan`, the active tools are:
...
1. `/gsd-plan` starts the interview and repo-grounding flow.
...
5. The planner calls `store-candidate-plan` with the candidate plan. The tool writes the plan to `.gsd-lite/candidate-plans/<id>.md`, persists a `gsd-stored-candidate-plan` entry, and returns `{id, path, iteration}`.
...
7. The planner passes the `candidatePlanId` (from step 5), raw review output, and the exact `planningContext` JSON into `validate-plan`. `validate-plan` resolves the id back to the stored plan, persists the context as `gsd-planning-context`, and persists the review cycle as `gsd-plan-review-cycle`.
...
10. `finalize-plan` writes `PLANS.md` only when the markdown exactly matches the latest clean reviewed candidate plan and a `gsd-planning-context` entry exists for the same review iteration.
...
- no `gsd-planning-context` entry exists for the latest review iteration
...
`finalize-plan` accepts warnings when `acceptWarnings: true` is passed; it then records the count in the `gsd-plan-finalized` entry.
